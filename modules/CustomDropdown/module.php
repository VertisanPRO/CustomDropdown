...
class CustomDropdown_Module extends Module
{
	...
	private $_language, $CustomDropdownLanguage;

	public function __construct($language, $pages, $INFO_MODULE)
	{
		$this->_language = $language;

		$this->CustomDropdownLanguage = $GLOBALS['CustomDropdownLanguage'];
	...
	public function onPageLoad($user, $pages, $cache, $smarty, $navs, $widgets, $template)
	{
		...
		PermissionHandler::registerPermissions('CustomDropdown', array(
			'admincp.csdropdown' => $this->CustomDropdownLanguage->get('general', 'group_permision')
		));
		...
		if (defined('FRONT_END')) {
			...
			$cs_dropdown_title = $this->CustomDropdownLanguage->get('general', 'title');
	...
	public function initialise()
	{
		...
		try {
			...
			$data = $queries->createTable("custom_dropdown", " `id` int(11) NOT NULL AUTO_INCREMENT, `dropdown_title` varchar(255) NOT NULL, `dropdown_location` int(1) NOT NULL, `dropdown_icon` varchar(255) NOT NULL, `dropdown_order` int(11) NOT NULL, `enabled` int(1) NOT NULL DEFAULT '1', PRIMARY KEY (`id`)", "ENGINE=InnoDB DEFAULT CHARSET=utf8");

			$data = $queries->createTable("custom_dropdown_pages", " `id` int(11) NOT NULL AUTO_INCREMENT, `dropdown_id` int(11) NOT NULL, `page_title` varchar(255) NOT NULL, `page_link` varchar(512) NOT NULL, `page_location` varchar(255) NOT NULL DEFAULT 'top', `page_target` varchar(20) DEFAULT NULL, `page_icon` varchar(255) DEFAULT NULL, `page_order` int(11) DEFAULT '0', `default_page_id` int(11) NOT NULL DEFAULT '0', PRIMARY KEY (`id`)", "ENGINE=InnoDB DEFAULT CHARSET=utf8");
		} catch (Exception $e) {
			echo $e->getMessage();
		}
